...
// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে (হুবহু ৩টি স্ক্রিনশটের ওরিজিনাল রুট সিঙ্ক ভাই ভাই)
...
// 🛫 ২. ৩ডি ডাইস রোল কোর এপিআই রাউট (জিরো ব্যালেন্স বাজি হ্যাকিং প্রতিরোধ কঠোর লক ভাই ভাই!)
app.post('/api/dice3d-shake', async (req, res) => {
    ...
    try {
        ...
        if (balResponse.data && balResponse.data.status === "ok" && balResponse.data.balance !== undefined && balResponse.data.balance !== null) {
        ...
        // 🔒 [কঠোর লক বর্ম]: পকেটে বাজি ধরার চেয়ে কম টাকা থাকলে বা একাউন্ট জিরো হলে বাজি ডিরেক্ট রিফিউজড ভাই ভাই!
        ...
        // 🚀 ৩. পেলোড ম্যাপিং লক (হুবহু ২য় স্ক্রিনশটের ১৩১-১৪৭ নম্বর লাইনের ওরিজিনাল পিউর JSON স্ট্রাকচার ভাই ভাই)
        let phpPayload = {
            action: dbAction,
            username: userId,
            amount: dbAmount,
        ...
        // আপনার api_callback.php ফাইলে ফাইনাল ফায়ারিং হিট (হুবহু ৩টি স্ক্রিনশটের ওরিজিনাল প্রোটোকল মেথড ভাই ভাই)
        const response = await axios.post(MAIN_SITE_URL + '/api_callback.php', phpPayload, { timeout: 30000 });

        // 🚀 [মাস্টার সিঙ্ক ফিক্সড]: ৩টি স্ক্রিনশটের ১৫২ নম্বর লাইনের নিয়ম অনুযায়ী response.data.status === "ok" চেক লক ভাই ভাই!
        if (response.data && response.data.status === "ok") {
            io.emit("balanceUpdate", { username: userId, balance: response.data.balance });

            // 🚀 [গ্র্যান্ড কিংস সিঙ্ক]: ৩টি স্ক্রিনশটের ১৫৭ নম্বর লাইনের নিয়ম অনুযায়ী response.data.balance ডিরেক্ট পাস লক ভাই ভাই!
            return res.json({
                success: true,
...
// ৩ডি ডাইস গেম ১২ নম্বর স্বাধীন পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
